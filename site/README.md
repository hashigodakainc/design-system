...
ビルドは `scripts/public-files.mjs` の許可リストだけを公開します。`AGENTS.md`、Gitメタデータ、MCP実装、サイト用スクリプトなど、許可リスト外のファイルは公開しません。フォントは `../assets/fonts/` のTTFを原本とし、ビルド時にWOFF2へ変換してWOFF2だけを公開します。フォントの配布条件を同梱するため、利用するフォントのOFL文書も公開対象に含めます。
