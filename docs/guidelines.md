...
- トークンは、値を表す第1層（プリミティブ）、用途を表す第2層（セマンティック）、部品固有の
  第3層（コンポーネント）で構成する。プリミティブは値由来の名前、用途名は第2層以上で使う。
- 参照方向は第3層から第2層、第2層から第1層または第2層に限定する。第3層から第1層への直接参照は、
  対応する意味が第2層にない場合だけ許容し、同じ値を別の部品でも必要とした時点で第2層へ昇格する。
- 色の第1層と第2層は `tokens/colors.json`、タイポグラフィの第1層とロールは
  `tokens/typography.json`、余白プリミティブと用途別レイアウトは `tokens/layout.json`、角丸・境界線・
  フォーカス輪郭のプリミティブは `tokens/shape.json`、第3層は `tokens/components.json` を正本とする。
- 第3層の名前は `{component}.{variant}.{state}.{property}` とし、実装クラス名に対応する
  `button.*`、`badge.*`、`menu.*`、`card.*` を使う。
- すべてのトークンに、その値または用途を説明する `description` を記載する。

## 色と面積

- 制作物は第2層または第3層だけを参照し、`color.neutral.*` や `color.blue.*` など第1層を直接参照しない。
...
- `typography.size.*`、`typography.weight.*`、`typography.line-height.*`、`typography.tracking.*` は
  ロールを構成する第1層であり、制作物から直接参照しない。
...
- 余白は定義済みのスケール（`spacing.*`）から、隣接する要素同士の関係が伝わる最小の値を選ぶ。
  独自の中間値を追加する前に、要素のグループ分けや情報階層を見直す。
- `spacing.*` は、制作物から直接参照できる唯一のプリミティブとする。ただし、生の数値より先に
  用途別レイアウトトークンで意図を表せないか確認する。
...
- セクション間隔は話題の切り替わりと優先度を示す。すべてを最大にせず、同じ主題の内部は
  小さい間隔でまとめる。
- ブレークポイント（`layout.breakpoint.*-max`）は端末ではなく、内容が窮屈になる地点で構造を
  切り替える共通境界として使う。値以下を名前に示したティアとして扱い、次のティアへ切り替える。
...
実装の正本は `styles/components.css`（`.hsg-button`、`.hsg-menu`、`.hsg-badge`、`.hsg-card` と
修飾クラス）である。見た目の仕様はCSSを読む。
