...
- 中心命題：**企業の意思に、AI時代の実装力を与える。**
- 会社レベルの正本：Hashigodaka Wiki「ハシゴダカの存在意義とブランドの骨格」
- デザインへの翻訳：[brand/foundation.md](brand/foundation.md)

コピー、写真、図解、モーションでは、顧客自身の意思、人が担う判断と責任、AIによって増幅される
実装力、合意した成果へ正しく届ける関係を中心にする。既存のサイトコピーは特定画面での表現であり、
会社の存在意義全体を表す確定コピーとしては扱わない。
...
- DesktopとMobileのサイズ
- 用途別のウェイト、行間、字間
- 選定書体D-I（Sora × LINE Seed JP）を用途別スタイルへ適用する定義

用途別スタイルとD-Iは新しい実装へ利用できる。Soraを基にした正式ワードマークの字形調整と
フォント配信を含むライセンス確認は別途必要である。詳細は
[foundations/typography.md](foundations/typography.md) を参照する。

## 利用可能な固有モチーフ

- 暫定メイン候補：01C-C
- 資産：`assets/motifs/01c-c.svg`
- 既定色：Primary `#00A0FF`
- 表現：単色、縦横比固定、回転・反転・輪郭改変なし

写真がない場面の主役となるブランドグラフィックとして検証利用できる。正式ロゴ、ファビコン、
ワードマークとの固定ロックアップにはまだ使わない。32px未満の小サイズと横組みは検証中である。
詳細は [brand/motif.md](brand/motif.md) を参照する。
...
- 角丸、罫線、影、モーション
- 01C-Cの最終曲線、32px未満の最小サイズ、D-Iワードマークとの固定ロックアップ
...
2. 選定したD-Iを用途別タイポグラフィへ適用し、Soraを土台に正式ワードマークを調整する。
3. 採用済みの余白、グリッド、コンテンツ幅を実装で検証する。
4. 01C-CをB2B LPと図解へ適用し、小サイズとD-I横組みを検証する。
5. Semantic Colorsを機能別に選ぶ。
6. 採用済みボタン・バッジ仕様を運用し、リンク、カード、ナビゲーション、図解などもコンポーネント化する。
7. 人間向けガイドとAI向け定義を同じ正本から配布し、別環境で再現性を検証する。
