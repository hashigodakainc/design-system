...
フォントファミリーはD-I（Sora × LINE Seed JP）へ確定した。H-J（Geologica × M PLUS 1）は
比較過程を再現するアーカイブにのみ残し、新しい実装の選択肢にはしない。正式ロゴの字形調整と
ライセンス確認は引き続き必要である。
...
- Soraは、英字ワードマークと英字だけの表示を検証する基準書体とする。
...
`data-hsg-font-pair="di"`を指定する。未指定時もD-Iを使う。`hj`は過去の比較画面を
再現する互換指定としてのみ残し、新しい制作物では使わない。
